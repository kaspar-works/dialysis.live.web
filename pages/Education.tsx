  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-slate-500">Expert-reviewed resources for your dialysis journey.</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {articles.map((article, idx) => (
          <div key={idx} className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden group cursor-pointer hover:shadow-xl transition-all">
             <div className="h-48 overflow-hidden relative">
                <img src={article.image} alt={article.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                <div className="absolute top-4 left-4">
                   <span className="bg-white/90 backdrop-blur px-3 py-1 rounded-full text-xs font-bold text-slate-800 uppercase tracking-wider">{article.category}</span>
                </div>
             </div>
             <div className="p-6">
                ...
                <h3 className="text-xl font-bold text-slate-800 mb-3 group-hover:text-sky-600 transition-colors">{article.title}</h3>
                <p className="text-slate-500 text-sm leading-relaxed mb-6">{article.excerpt}</p>
                <button className="flex items-center gap-2 text-sky-600 font-bold text-sm uppercase tracking-widest hover:gap-3 transition-all">
                   Read Full Story
      ...
      <div className="bg-sky-600 p-10 rounded-[2.5rem] text-white flex flex-col items-center text-center">
         ...
         <p className="text-sky-100 mb-8 max-w-lg">Get weekly updates on renal health, new recipes, and management tips delivered straight to your inbox.</p>
         <div className="w-full max-w-md flex flex-col sm:flex-row gap-3">
            <input type="email" placeholder="Enter your email" className="flex-1 px-6 py-4 rounded-2xl text-slate-800 font-medium focus:outline-none" />
