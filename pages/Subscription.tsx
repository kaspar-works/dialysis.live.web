...
const Subscription: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    setError(null);
    try {
      const [subData, usageData, plansData, paymentMethodsData] = await Promise.all([
        getCurrentSubscription(),
        getUsageStats(),
        getPlans(),
  ...
  const handleChangePaymentMethod = async (paymentMethodId: string) => {
    if (!confirm('Use this card for your subscription?')) return;
      ...
      {activeTab === 'overview' ? (
        <div className="space-y-8">
          ...
          {subscription && subscription.plan !== 'free' && (
            <div>
              <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-4">Payment Methods</h2>
              <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden">
                {paymentMethods.length === 0 ? (
                  <div className="p-8 text-center">
                    <div className="w-16 h-16 bg-slate-100 dark:bg-slate-700 rounded-2xl flex items-center justify-center mx-auto mb-4">
                      <ICONS.CreditCard className="w-8 h-8 text-slate-400" />
                    </div>
                    <p className="text-slate-600 dark:text-slate-400 mb-4">No payment methods on file</p>
                    <button
                    ...
                    {paymentMethods.map((pm, index) => (
                      <div key={pm.id} className="p-4 flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          {index !== 0 && (
                            <button
                              onClick={() => handleChangePaymentMethod(pm.id)}
                              disabled={isProcessingPayment}
                              className="px-3 py-1.5 text-sm font-medium text-sky-600 dark:text-sky-400 hover:bg-sky-50 dark:hover:bg-sky-500/10 rounded-lg transition-colors disabled:opacity-50"
                            >
                              Use This Card
                            </button>
                          )}
                          <button
                            ...
                            disabled={isProcessingPayment}
                            className="px-3 py-1.5 text-sm font-medium text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-500/10 rounded-lg transition-colors disabled:opacity-50"
                          >
                            Remove
                          </button>
                        </div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-3 flex items-center gap-1">
                ...
                Payments are securely processed by Stripe. We never store your full card details.
              </p>
            </div>
          )}
        </div>
